...
#define H8_3069F_TMR01 ((volatile struct h8_69_tmr *)0xffff80)
#define H8_3069F_TMR23 ((volatile struct h8_69_tmr *)0xffff90)
...
static struct {
	volatile struct h8_69_tmr *tmr;
} regs[TIMER_TMR_NUM] = {
	{H8_3069F_TMR01}, {H8_3069F_TMR23},
};

//Timer start
int timer_start(int index, int msec) {
	volatile struct h8_69_tmr *tmr = regs[index].tmr;
	int count;
	...
	tmr->tcnt = 0;
	tmr->tcora0 = count;
	tmr->tcr0 |= H8_3069F_TMR_TCR_CMIEA; //Interrupt Enable
...
int timer_cancel(int index) {
	...
	tmr->tcr0 &= ~H8_3069F_TMR_TCR_CMIEA; //Interrupt Disable
