...
static int ip(struct Packet *ip, struct IP_Header *copy, uint32 srcip, uint32 dstip) {
	...
	ip->ip_Ver = 0x45;
	ip->ip_ServiceType = 0;
	ip->ip_TotalLength = PACKET_SIZE - 14;
	
	if(ip->ip_ID == 0xfffe) i = 0;
	ip->ip_ID = i;
...
/*	for(i=0;i<6;i++) {
		puthex(eth->srcMAC[i],2); puts("\n");
		puthex(eth->dstMAC[i],2); puts(":");
	}
...
#define tIPADDR 0xc0a80110 //192.168.1.16
	uint32 srcip = IPADDR;
