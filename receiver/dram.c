...
static uint32 chkdram_val0(uint32 *addr) {  return 0x55555555UL;  }
static uint32 chkdram_val1(uint32 *addr) {  return 0xaaaaaaaaUL;  }
static uint32 chkdram_val2(uint32 *addr) {  return 0x00000000UL;  }
static uint32 chkdram_val3(uint32 *addr) {  return 0xffffffffUL;  }
static uint32 chkdram_val4(uint32 *addr) {  return (uint32)addr;  }
static uint32 chkdram_val5(uint32 *addr) {  return ~(uint32)addr; }
...
int cleandram(){
	uint32 *p;
	puts("[DRAM cleaning...");
	for(p=(uint32 *)DRAM_START; p<(uint32 *)DRAM_END; p++) *p = 0;
	puts("done.]\n");
	return 0;
}
