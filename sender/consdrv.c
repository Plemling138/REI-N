...
static int consdrv_command(struct consreg *cons, sys_thread_id_t id, int index, int size, char *command) {
  switch(command[0]) {
  case CONSDRV_CMD_USE:
    cons->id = id;
    cons->index = command[1] - '0';
    cons->send_buf = sys_memalloc(CONS_BUF_SIZE);
    cons->recv_buf = sys_memalloc(CONS_BUF_SIZE);
    cons->send_len = 0;
    cons->recv_len = 0;
